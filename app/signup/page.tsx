...
function StatusMsg({ state }: { state: CheckState }) {
  ...
  return (
    <p style={{ margin: '2px 0 0', fontSize: 12, color: state === 'ok' ? '#22c55e' : '#ef4444' }}>
...
export default function SignupPage() {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    if (checks.nickname !== 'ok' || checks.userId !== 'ok' || checks.email !== 'ok') {
      alert('닉네임, 아이디, 이메일 중복확인을 완료해주세요.')
      return
    }
    if (!phoneCertified) {
      alert('핸드폰 본인인증을 완료해주세요.')
      return
    }
    // TODO: 실제 회원가입 API 연동
    router.push('/login')
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '40px 16px' }}>
      <div style={{ width: '100%', maxWidth: 460, padding: '40px 32px', background: '#fff', borderRadius: 16, boxShadow: '0 4px 24px rgba(0,0,0,0.08)' }}>
        <h2 style={{ textAlign: 'center', marginBottom: 28, fontSize: 24, fontWeight: 700 }}>회원가입</h2>
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
            <div style={{ display: 'flex', gap: 8 }}>
              <input
                ...
                style={inputStyle}
              />
              <button type="button" style={checkBtnStyle} onClick={() => handleCheck('nickname')}>
                중복확인
              </button>
            </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
            <div style={{ display: 'flex', gap: 8 }}>
              <input
                ...
                style={inputStyle}
              />
              <button type="button" style={checkBtnStyle} onClick={() => handleCheck('userId')}>
                중복확인
              </button>
            </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
            ...
            />
            {form.confirm && (
              <p style={{ margin: '2px 0 0', fontSize: 12, color: form.password === form.confirm ? '#22c55e' : '#ef4444' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
            <div style={{ display: 'flex', gap: 8 }}>
              <input
                ...
                style={inputStyle}
              />
              <button type="button" style={checkBtnStyle} onClick={() => handleCheck('email')}>
                중복확인
              </button>
            </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
            <div style={{ display: 'flex', gap: 8 }}>
              <input
                ...
                style={inputStyle}
              />
              <button type="button" style={checkBtnStyle} onClick={handlePhoneCert}>
                본인인증
              </button>
            </div>
            {phoneCertified && (
              <p style={{ margin: '2px 0 0', fontSize: 12, color: '#22c55e' }}>본인인증이 완료되었습니다.</p>
            )}
          </div>
